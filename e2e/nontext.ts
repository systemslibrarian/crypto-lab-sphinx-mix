 ...
 * defines the boundary token `--control-border` and applies it to `.btn`,
 * `.tab-btn`, `.check-opt`, `.pill`, `.meter`, `.legend-swatch` and every
 * `input`/`select` — but tokens are discarded by overrides, and measuring
 * rather than trusting that is the whole reason this oracle exists. Four
 * shapes on this page override or bypass the token: the SELECTED `.tab-btn`
 * repaints its border the same colour as its own accent fill, so it has no
 * edge of its own and lives or dies by fill-versus-surround; `.btn-danger`
 * and the `.pill-*` states redraw theirs from a semantic tone; `.hopnode`
 * swaps between `--control-border`, `--accent`, `--ok` and `--bad` depending
 * on whether that hop peeled the packet or killed it; and `.histo-bar` and
 * `.meter-fill` are fills with no border at all. This oracle judges each
 * control as painted, at every driven state — including the dead-hop and
 * failure-code recolourings only the drive reaches.
 *
 * The generated-content half is inert in this repo today — the stylesheet
 * declares no `content` at all. The `<summary>` disclosure triangles are
 * `::marker`, the UA's own, and the custom `<select>` chevron is drawn with
 * two `linear-gradient` background layers ON THE ELEMENT rather than in a
 * pseudo-element, so it is judged by the control-boundary half instead. This
 * half runs anyway, at every state, so that stays a measurement rather than a
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * per-byte hex spans a diffed `hexDump` emits (~192 siblings per Peel
     * measurement panel) and act 6's fourteen-row observer notebook — all of
     * them re-walking the same ancestors up to `<body>`. Without the caches
     * the pass re-reads the same computed styles and rects tens of thousands
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // load-bearing assumption. It is also the fix 129 of 131 `nontext.ts`
      // files in this fleet still lack, which is why this one was copied from
      // crypto-lab-schnorr-forge rather than from an arbitrary recent lab.
      //
      ...
      // does not use the spacer idiom today — every `.tab-btn` paints a real
      // `--control-border` in its unselected state and an `--accent` one when
      // selected — but the guard stays, because the first `border: 1px solid
      // transparent` anyone adds would otherwise be counted as a delineator
      // and report a phantom 1.00:1.
